...
fn main() {
    ...
    let handle_r1 = thread::spawn(move || {
        pause_ms(100);
        for msg in rx {
            pause_ms(100);
    ...
    let handle_r2 = thread::spawn(move || {
        pause_ms(50);
        for msg in rx2 {
            pause_ms(100);
    ...
    let vec = vec![11, 22, 32, 43, 55];
    for elem in vec {
        let msg = String::from("Main thread sends ") + &*elem.to_string();
        tx.send(msg).unwrap();
    }
    drop(tx);

    handle_r1.join().unwrap();
    handle_r2.join().unwrap();
